...
class ProbabilisticReconciliation:
  ...
  def sample_reconciled_posterior_predictive(
    ...
    def _logprob_fn(b):
      ...
      lp = self._forecaster.predictive_posterior_probability(rng_key, y, x)
      return jnp.sum(lp)
  ...
  def fit_reconciled_posterior_predictive(
    ...
    def _step(state, y_batched, y_predictive_batched):
      def loss_fn(params):
        ...
        y_reconciled_pred = self._grouping.all_timeseries(b_reconciled_pred)
        loss = _loss(
          y_batched,
          y_reconciled_pred[:, [0], :, :],
          y_reconciled_pred[:, [1], :, :],
    ...
    y_predictive = predictive.sample(seed=rng_key, sample_shape=(n_samples,))
