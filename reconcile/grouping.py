...
class Grouping:
  ...
  def _gts_create_g_mat(self):
    ...
    for i in range(cn.shape[1]):
      ...
      tmp_groups = [None] * nr1
      for j in range(nr1):
        tmp_groups[j] = self._paste0(bigroups[0][j, :], bigroups[1][0, :])
        if nr2 >= 2:
          for k in range(1, nr2):
            tmp_groups[j] = np.vstack(
              [
                tmp_groups[j],
                self._paste0(bigroups[0][j, :], bigroups[1][k, :]),
              ]
            )
  ...
  def _hts_create_nodes(self):
    ...
    for i in range(1, len(unique_tokens_per_level.keys())):
      ...
      els = [":".join(el.split(":")[:i]) for el in unique_tokens_per_level[i]]
  ...
  def _hts_create_g_mat(out_edges_per_level):
    ...
    if len(out_edges_per_level) > 1:
      ...
      repcount = list(level.values()) if isinstance(level, dict) else [level]
      for i in np.arange(len(out_edges_per_level) - 1, 0, -1):
        ...
        times = list(times.values()) if isinstance(times, dict) else [times]
        x = pd.DataFrame(
          {
            "r": repcount,
            "g": np.repeat(np.arange(n_elements_per_level[i]), times),
          }
        )
        x = x.groupby("g").sum()
